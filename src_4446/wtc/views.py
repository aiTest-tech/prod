...
from scrutiny.views import ScrutinyDepartmentRoutingView
from sentiment.views import AnalyzeSentiment
from scrutiny.models import Data
from scrutiny.serializers import ScrutinyRequestSerializer
from sentiment.serializers import AnalyzeSerializer
from .serializers import WTCSerializer
from .models import WTC, Data
from auth_app.models import Project
from deep_translator import GoogleTranslator
...
# Helper Functions
def translate_text(text, source_lang="gu", target_lang="en"):
    """Translate text using scrutiny's GoogleTranslator implementation."""
    try:
        return GoogleTranslator(source=source_lang, target=target_lang).translate(text)
    except Exception as e:
        return f"Translation failed: {str(e)}"

# Main API View for Text Data
class TextDataView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, *args, **kwargs):
        ...
        wtc_message = serializer.validated_data['message']
        wtc_lang = serializer.validated_data.get('lang', 'en')

        try:
            # Create Data record
            data = Data.objects.create(
                audio_path="",  # Assuming not needed
                text=wtc_message,
                etext=""  # Optionally, store additional translations if necessary
            )
            ...
            project = Project.objects.filter(user=user).first()
            if not project:
                return Response({"error": "No project found for this user."}, status=status.HTTP_404_NOT_FOUND)

            # Save WTC entry
            wtc_entry = WTC.objects.create(
                message=wtc_message,
                user=user,
                project=project,
                data_id=data.id  # Store the Data ID in WTC
            )

            # Pass the data_id to the sentiment view
            sentiment_request_data = {"data_id": data.id}
            sentiment_response = AnalyzeSentiment().post(request)

            if sentiment_response.status_code != status.HTTP_200_OK:
                return sentiment_response  # Return sentiment response if it fails

            sentiment_data = sentiment_response.data
            sentiment_label = sentiment_data.get('sentiment')
            sentiment_score = sentiment_data.get('gravity')

            # Pass the data_id to the scrutiny view
            scrutiny_request_data = {"data_id": data.id}
            scrutiny_response = ScrutinyDepartmentRoutingView().post(request)

            if scrutiny_response.status_code != status.HTTP_201_CREATED:
                return scrutiny_response  # Return scrutiny response if it fails

            scrutiny_data = scrutiny_response.data
            department = scrutiny_data.get('department')
            scrutiny_decision = scrutiny_data.get('scrutiny_decision')

            # Update the WTC entry with the sentiment and scrutiny information
            wtc_entry.sentiment_cal_gra = sentiment_score
            wtc_entry.sentiment_cal_pol = sentiment_label
            wtc_entry.depr_rout = department
            wtc_entry.lo_sc = scrutiny_decision
            wtc_entry.save()
            ...
            response_data = {
                ...
                "scrutiny_decision": scrutiny_decision
            }
...
class RequestsPerDayView(APIView):
    def get(self, request):
        ...
        return Response(requests_per_day, status=status.HTTP_200_OK)
...
class ScrutinySummaryView(APIView):
    def get(self, request):
        scrutiny_summary = WTC.objects.values('lo_sc').annotate(count=Count('id'))
        return Response(scrutiny_summary, status=status.HTTP_200_OK)


class ModeDistributionView(APIView):
    def get(self, request):
        mode_distribution = WTC.objects.values('mode').annotate(count=Count('id'))
        return Response(mode_distribution, status=status.HTTP_200_OK)


class DeprRoutDistributionView(APIView):
    def get(self, request):
        depr_rout_distribution = WTC.objects.values('depr_rout').annotate(count=Count('id'))
        return Response(depr_rout_distribution, status=status.HTTP_200_OK)
...
class AnalyticsAPIView(APIView):
    def get(self, request):
        ...
        data = {
            ...
            "todays_applications": WTC.objects.filter(created_at__date=today).count(),
            "pending_applications": WTC.objects.filter(pending=True).count(),
            "daily_average_applications": round(WTC.objects.filter(created_at__date__gte=last_week).count() / 7, 2),
        }
        return Response(data, status=status.HTTP_200_OK)
...
class TypeDistributionView(APIView):
    def get(self, request):
        ...
        for t in allowed_types:
            filters |= Q(type__iexact=t) | Q(type__iexact=f"{t}s")
        type_distribution = WTC.objects.filter(filters).values('type').annotate(count=Count('id'))
        return Response(type_distribution, status=status.HTTP_200_OK)
...
class WTCStatisticsAPIView(APIView):
    def get(self, request):
        ...
        accuracy = round((matched_decisions / machine_decisions) * 100, 2) if machine_decisions else 0
        data = {
            "total_accepted": total_accepted,
            "total_rejected": total_rejected,
            "accuracy": accuracy,
